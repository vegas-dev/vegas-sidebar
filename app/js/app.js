...
const _cross = '<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"' +
...
class VGSidebar extends BaseModule{
		...
		if (isObject(mParams) && !isEmptyObj(mParams)) {
			for (const datum in mParams) {
				...
				if (!(datum in this.paramsDefault)) {
					let [group, key] = datum.split('-');

					if (group && key && isObject(this.paramsDefault[group]) && key in this.paramsDefault[group]) {
						if (!isObject(mParams[group])) {
							mParams[group] = {};
						}
						mParams[group][key] = value;
					}
	...
	static makeInit(btn) {
		btn.addEventListener('click', () => {
			...
			if (target && typeof target === 'string') {
				...
				let sidebar = VGSidebar.getInstance(target);
				if (!sidebar) sidebar = new VGSidebar(target, arg);
				sidebar.toggle();
			}
	...
	_addEventListener() {
		...
		[..._this.element.querySelectorAll('[data-vg-dismiss="sidebar"]')].forEach(function (cross) {
		...
		if (_this.params.keyboard) {
			...
			document.removeEventListener('keyup', _this._onKeyUp);
